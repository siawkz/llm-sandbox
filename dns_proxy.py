...
class WhitelistDNSResolver:
    def __init__(self, whitelist_file, upstream_dns="8.8.8.8"):
        ...
        print(
            f"[{datetime.now().isoformat()}] DNS proxy started with {len(self.whitelist)} whitelisted domains"
        )
        sys.stdout.flush()
    ...
    def load_whitelist(self, whitelist_file):
        try:
            ...
            print(
                f"[{datetime.now().isoformat()}] ERROR: Whitelist file {whitelist_file} not found"
            )
            sys.stdout.flush()
            return set()
